...
try {
    ...
    # (agents/intake.md) already said so in prose ("does not... commit, push,
    # or create another card"), and on 2026-08-28 a haiku run of it committed
    # and pushed a self-authored card to the shared branch anyway, under an
    # ambiguous "do the real job" framing, in the SAME turn it was told twice
    # not to (reports/personas/_LOG.md, "A HAIKU SUBAGENT COMMITTED AND
    # PUSHED..."). A written "does not" is not a boundary a model reliably
    # holds; this is the mechanical one. Keyed on `agent_type` from the
    # PreToolUse payload (present only inside a subagent call, doc-verified
    # 2026-08-28: https://code.claude.com/docs/en/hooks.md), so this can
    # never fire for the main thread or any other subagent, including ones
    # that legitimately commit (cleanup-crew, fast-lane).
    $agentType = ""
    if ($payload.agent_type) { $agentType = [string]$payload.agent_type }
    if ($agentType -eq "intake" -and $unquoted -match '(^|\s)git(\s|$)|(^|\s)git\s' -and $unquoted -match '(^|\s)(commit|push)(\s|$)') {
        $reasons += "the intake agent tried to $(if ($unquoted -match '\bpush\b') { 'push' } else { 'commit' }), which its own scope (agents/intake.md) forbids outright, it says plainly: does not commit, push, or create another card. This is not a permission gap to route around: write the order file and stop, the quarterback reviews and launches. If this order genuinely needs to ship, report that back instead of committing it yourself"
    }
