...
**Layout follows the memory bank:** `shared/` holds the rules every tool
reads (canonical copy in the private bank), `claude/` holds the Claude-only
rules. `opencode/` is reserved for the opencode workbench and ships empty
here. Keep files byte-identical with the bank copies in `shared/`; the
mirror check polices it.
...
`claude/git-github.md`, `claude/session-dirs.md` and `shared/documentation.md`; that directory is
